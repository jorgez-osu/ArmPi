...
# coding=utf8
"""
Demo: block stacking using PerceptionPipeline + MotionController.

Detects the largest block among red/green/blue and uses MotionController
to stack blocks for each color on a shared stacking area.
"""
...
def main():
    ...
    try:
        while True:
                ...
                if detection is not None:
                    ...
                    if distance < 0.5:
                        ...
                        if start_count_t1:
                            ...
                            t1 = time.time()
                        if time.time() - t1 > 0.5 and count > 0:
            ...
            cv2.imshow("Motion Stacking Demo", out)
            if cv2.waitKey(1) == 27:
                break
            ...
            if picking and stable_point is not None and chosen_color != "None" and (
                ...
                def do_motion():
                    ...
                    # 3) Grasp using refined pose, then place on current stack level
                    motion.grasp_block(refined_x, refined_y, refined_rot, z_override=2.0)
                    drop_x, drop_y, drop_z = motion.get_next_stack_target(color)
